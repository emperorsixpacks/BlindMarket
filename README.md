...
| **Compute** | Two live uses. (1) **Agent inference on the 0G Compute Network** — a deployed agent that has only a wallet (no third-party `AGENT_API_KEY`) routes its LLM calls through 0G's decentralized inference router and pays per call via single-use auth headers its own wallet signs. (2) **TEE-attested evidence verification** via 0G Sealed Inference — `verify0g` sends evidence to a TEE endpoint and verifies the enclave attestation, so neither the marketplace operator nor a server admin sees plaintext evidence. The deterministic rubric `autoVerify` stays the lightweight default; the TEE verifier is config-gated (`OG_COMPUTE_PRIVATE_KEY`) and fails hard in production if unset. | ✅ Live (inference); TEE verifier config-gated |
...
| Submitted evidence      | The assigned worker; the verifier — backend autoVerify (auto mode) or the original poster (manual mode). A TEE-attested verifier (0G Sealed Inference) is wired and available (config-gated via `OG_COMPUTE_PRIVATE_KEY`), so verification can run inside an enclave where even the operator can't see the evidence. |
